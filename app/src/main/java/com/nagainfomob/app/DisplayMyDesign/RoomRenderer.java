...
//		cam.setFarPlane(1000);
		...
		cam.setNearPlane(10);
		cam.setFieldOfView(100); //important - zoom view
	...
	private String saveScreenShot(Bitmap bmp) {
		...
		path = Environment.getExternalStorageDirectory().getAbsolutePath()
				+ "/DMD/ScreenShots/" + currentProject + "/";
	...
	public void initScene() {
		super.initScene();
		DirectionalLight spotLight = new DirectionalLight(0, 0, 1);
...
//		getCurrentScene().setBackgroundColor(0xffffff);
		...
		// getCurrentScene().setUsesCoverageAa(true);
		{
			...
			plane.setName("front");
			plane.setRotY(180);
//			plane.setColor(R.color.colorAccent);
...
//			plane.setVisible(false);
			...
			/*DirectionalLight key = new DirectionalLight(-2,-2, 2);
			key.setLookAt(group.getPosition());
			key.setPower(9);
			getCurrentScene().addLight(key);

			getCurrentCamera().setPosition(3,3,3);
			getCurrentCamera().setLookAt(group.getPosition());*/

//			getCurrentCamera().setLookAt(group.getPosition());  //*Important - camera View
		...
		try {
			objParser.parse();
			mObjectGroup = objParser.getParsedObject();
			mObjectGroup.setName("Monkey");
			mPicker.registerObject(mObjectGroup);
			mObjectGroup.setScale(45);
			mObjectGroup.setMaterial(material);
			mObjectGroup.setColor(0xff9955);
			addChild(mObjectGroup); mObjectGroup.setColor(Color.BLACK);
			mObjectGroup.setPosition(0, -0.5*scale, -0.25*scale);*/


		LoaderOBJ objParser = new LoaderOBJ(mContext.getResources(),mTextureManager, R.raw.sink);
...
//			Object = parser.getParsedObject();
			...
			Material mSimpleMat = new Material(); //ADDED
			mSimpleMat.enableLighting(true);
...
//			mObjectGroup.setMaterial(material); //ADDED
//			mObjectGroup.setColor(0xffffffff); //ADDED
			mObjectGroup.setColor(Color.BLUE);
			mObjectGroup.setRotY(180);
			mObjectGroup.setScale(10); //ADDED
			mObjectGroup.setPosition(0,0,-150);

			group.getChildAt(1).addChild(mObjectGroup);
...
//		plane.setTransparent(true);
		...
		try {
			...
			if (imgFile.exists()) {
				Bitmap myBitmap = BitmapFactory.decodeFile(imgFile
						.getAbsolutePath());// storage/emulated/0/DMD/hdhs/front.png
				// Bitmap myBitmap =
				// BitmapFactory.decodeFile("/storage/emulated/0/DMD/hdhs/front.png");
	...
	public void stopMovingSelectedObject() {
		...
		Material material = new Material();
		try {
			material.addTexture(new Texture("checkerboard", R.drawable.dmd_wall_bg));
			material.setColorInfluence(0);
		} catch (TextureException e) {
			e.printStackTrace();
		}
...
//		mSelectedObject.setX(10);
//		mSelectedObject.setY(10);
	}
